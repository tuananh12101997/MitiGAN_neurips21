...
class PGD:
    # def __init__(self, model, normalizer, alpha=0.05, epsilon=0.1, iters=100):  # MNIST
...
class TransferAttack(object):
    ...
    def forward(self, inputs, targets_adversarial, opt):
        ...
        hook_module = list(self.model.children())[-1].register_forward_hook(hook_fn)

        for k in range(self.iters):
            inputs_adversarial = inputs_adversarial.clone().detach()
            inputs_adversarial.requires_grad = True
            container = []
            preds = self.model(inputs_adversarial)
            last_feature = container[0]
            adversarial_mitigated = self.G(inputs_adversarial)
            container = []
            preds_miti = self.model(adversarial_mitigated)
            last_feature_miti = container[0]
...
class SignatureAttack(object):
    ...
    #def __init__(self, model, normalizer, signature, epsilon=0.01, alpha=3, iters=100, weight=0.005):  # Tinyimagenet   
