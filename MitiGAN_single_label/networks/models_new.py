...
class Normalizer:
    ...
    def _get_normalizer(self, opt):
        ...
        elif opt.dataset == "gtsrb" or opt.dataset == "celeba" or opt.dataset == 'TinyImageNet':
            normalizer = Normalize(opt, [0.5, 0.5, 0.5], [0.5, 0.5, 0.5])
        else:
...
class NetC_MNIST_MITI(nn.Module):
    def __init__(self):
        super(NetC_MNIST_MITI, self).__init__()
        self.conv1 = nn.Conv2d(1, 16, (5, 5), 1, 0)
        self.relu2 = nn.ReLU(inplace=True)
        self.dropout3 = nn.Dropout(0.1)

        self.maxpool4 = nn.MaxPool2d((2, 2))
        self.conv5 = nn.Conv2d(16, 32, (5, 5), 1, 0)
        self.relu6 = nn.ReLU(inplace=True)
        self.dropout7 = nn.Dropout(0.1)

        self.maxpool5 = nn.MaxPool2d((2, 2))
        self.flatten = nn.Flatten()
        self.linear6 = nn.Linear(32 * 4 * 4, 128)
        self.relu7 = nn.ReLU(inplace=True)
        self.dropout8 = nn.Dropout(0.1)
        self.linear9 = nn.Linear(128, 10)
